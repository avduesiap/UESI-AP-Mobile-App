    ...
    return (
        <TouchableOpacity style={[styles.productBox]} onPress={() => props.navToDetail(data)}>
            ...
            <View style={[styles.infromationView, props.fromDynamic ? { width: wp('36%') } : null]}>
                ...
                {data.stock_status_id!=2?<View style={styles.priceView}>
                    {
                        special > 0 ?
            ...
            {
                data.stock_status_id==2 && <View style={[GlobalStyles.outstockview,{marginTop:-10,marginLeft:-8}]} >
            ...
            {
                props.userAuth?(wishlistArr && wishlistArr.length > 0 && wishlistArr.includes(data.id) ? <TouchableOpacity style={[GlobalStyles.FavCircle, props.fromDynamic && { left: wp('27%') }]} onPress={() => props.addToWishlist(data.id)} >
