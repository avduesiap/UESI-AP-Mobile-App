...
import { View, Text,SafeAreaView, ScrollView, StyleSheet,FlatList,Dimensions, TouchableOpacity,ActivityIndicator, Platform, Image} from 'react-native';
import {
...
const MaterialsScreen = (props) => { 
  const [formData, setData] = React.useState({ type:null, firstName: null, lastName: null, email: null, mobileNumber: null, password: null, cpassword: null, submited: false, type: null, message: null, loading: false });
    ...
    const ItemSeparatorView = () => {
      ...
      const openPdfView = (item )=>{
        props.navigation.navigate('PdfViewScreen',{material:item}); 
      }
      const ItemView = ({ item,type }) => {
        return (
          <>  
          <View style={{display:'flex',flexDirection:'row',alignItems:'center'}}>
          <Image source={require('../assets/images/pdf-icon.png')} style={{width: 25, height: 25}} />
              <Text style={styles.itemTelugu} onPress={() => openPdfView(item)}>
                {item.title}
              </Text>
         </View> 
      ...
      <ScrollView style={{marginBottom:30}}>
          {/* <FlatList style={{paddingBottom:50}}
            ...
            <View style={{padding:10}}>
                    <View onPress={()=>openSongPage(item)}>
                    <Text style={{paddingTop:5, paddingLeft:5,color:'#000', fontFamily:Fonts.Font_Medium, fontSize:15}}>{item.title}</Text>
                    </View>
...
export default connect(mapStateToProps, mapDispatchToProps) (MaterialsScreen);
